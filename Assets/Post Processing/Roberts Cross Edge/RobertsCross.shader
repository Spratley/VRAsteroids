Shader "Hidden/PostProcess/RobertsCrossEdge"
{
    SubShader
    {
        ...
        Pass
        {
			...
			float4 _Color;
			float _Scale;
			float _DepthNormalThresh;
			float _DepthNormalThreshScale;
			float _EdgeThreshDepth;
			float _EdgeThreshNorm;
			...
			float CalculateEdgeDepth(float2 texcoord, float3 viewSpaceDir)
			{
				//Calculate uvs
				...
				float depthFiniteDif0 = depth1 - depth0;
				float depthFiniteDif1 = depth3 - depth2;
				
				float depthThresh = _EdgeThreshDepth * depth0;
				//Roberts cross operator.
				float edgeDepth = sqrt(pow(depthFiniteDif0, 2) + pow(depthFiniteDif1, 2)) * 100;
			...
			{
				float edgeDepth = CalculateEdgeDepth(i.texcoord, i.viewSpaceDir);
